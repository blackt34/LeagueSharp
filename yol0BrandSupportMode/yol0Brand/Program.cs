...
namespace yol0Brand
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("support", "Support Mode")).SetValue(false);
        ...
        {
            if (((Obj_AI_Base)_orbwalker.GetTarget()).IsMinion && /*args.Unit.IsMinion &&*/ Config.Item("support").GetValue<bool>()) args.Process = false;
        }
        ...
        {
            Utility.HpBarDamageIndicator.Enabled = Config.SubMenu("Drawing").Item("drawDamage").GetValue<bool>();
        ...
        {
            return (Player.Mana - spell.Instance.ManaCost) / Player.MaxMana >= Config.SubMenu("Harass").Item("mana").GetValue<Slider>().Value / 100;
        }
        ...
        private static double GetDamage(Obj_AI_Hero target)
        {
            ...
            var qMana = _Q.Instance.ManaCost;
            var wMana = _W.Instance.ManaCost;
            var eMana = _E.Instance.ManaCost;
            var rMana = _R.Instance.ManaCost;
            var totalMana = 0.0;
